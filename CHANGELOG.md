...
- フッターの更新ボタンを、コマンドをコピーするだけの UI から
  「アップデート」→「アップデートしますか? はい/いいえ」→ 実行中（不確定インジケーター）
  → 「再起動」の一連のフローに変更。brew 導入時は `brew update && brew upgrade --cask moost`
  をアプリが実際に実行し、完了後は手動の「再起動」クリックで新版を起動する
  （自動再起動はしない。brew の CLI 出力に数値の進捗はないため進捗バーは不確定表示）。
  手動導入の場合はこれまでどおりリリースページを開くのみ。「アップデートしますか?」で
  「いいえ」を選ぶと、自動実行の代わりにコマンドをコピーするかを尋ねる導線を追加
  （実行そのものを避けたい人向け。コピー後は自動で最初の状態に戻る）
